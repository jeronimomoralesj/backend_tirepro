/**
 * Fast Merquellantas rebuild — bulk SQL upserts.
 * SAFETY: never modifies a row whose companyId is in the Remax set.
 */
...
async function main() {
  ...
  const remaxLinks = await retry(() => prisma.distributorAccess.findMany({ where: { distributorId: REMAX_DIST }, select: { companyId: true } }), 'init.remax');
  const remaxIds = new Set(remaxLinks.map((l) => l.companyId));
  const remaxArr = [...remaxIds];
  console.log(`Remax clients (UNTOUCHABLE): ${remaxIds.size}`);

  const merqueLinks = await prisma.distributorAccess.findMany({ where: { distributorId: MERQUE_DIST }, select: { companyId: true } });
  ...
  const safeCos = await prisma.company.findMany({ where: { id: { in: [...safeIds] } }, select: { id: true, name: true } });
  ...
  for (let i = 0; i < vehRows.length; i += VBATCH) {
    ...
    const sql = `
      ...
      VALUES ${values.join(', ')}
      ON CONFLICT ("externalSourceId") DO UPDATE SET
        "companyId" = CASE WHEN "Vehicle"."companyId" = ANY(${remaxP}) THEN "Vehicle"."companyId" ELSE EXCLUDED."companyId" END,
        placa = EXCLUDED.placa,
        tipovhc = EXCLUDED.tipovhc,
        configuracion = COALESCE(EXCLUDED.configuracion, "Vehicle".configuracion),
        "kilometrajeActual" = EXCLUDED."kilometrajeActual",
        "estadoOperacional" = CASE WHEN "Vehicle"."companyId" = ANY(${remaxP}) THEN "Vehicle"."estadoOperacional" ELSE EXCLUDED."estadoOperacional" END,
        "fueraDeOperacionDesde" = EXCLUDED."fueraDeOperacionDesde",
        "originalClient" = EXCLUDED."originalClient",
        "sourceMetadata" = EXCLUDED."sourceMetadata",
        "updatedAt" = NOW()
      WHERE "Vehicle"."companyId" IS NULL OR "Vehicle"."companyId" <> ALL(${remaxP})`;
    const isUniqueViolation = (e: any) => e?.code === 'P2002' || e?.meta?.code === '23505';
    try {
      await retry(() => prisma.$executeRawUnsafe(sql, ...params), 'veh.bulk');
    } catch (e: any) {
      if (!isUniqueViolation(e)) throw e;
      // Per-row fallback. For (companyId, placa) conflicts, update the
      // existing row by its current extId without changing it.
      for (const r of slice) {
        try {
          await retry(() => prisma.$executeRawUnsafe(
            `INSERT INTO "Vehicle" (id, "externalSourceId", "companyId", placa, tipovhc, configuracion, "kilometrajeActual", "estadoOperacional", "fueraDeOperacionDesde", "originalClient", carga, "pesoCarga", "sourceMetadata", "updatedAt")
             VALUES (gen_random_uuid()::text, $1,$2,$3,$4,$5,$6,$7::"VehicleOperationalState",$8,$9,'seca',0,$10::jsonb,NOW())
             ON CONFLICT ("externalSourceId") DO UPDATE SET "companyId"=EXCLUDED."companyId", placa=EXCLUDED.placa, "kilometrajeActual"=EXCLUDED."kilometrajeActual", "estadoOperacional"=EXCLUDED."estadoOperacional", "originalClient"=EXCLUDED."originalClient", "sourceMetadata"=EXCLUDED."sourceMetadata", "updatedAt"=NOW()
             WHERE "Vehicle"."companyId" IS NULL OR "Vehicle"."companyId" <> ALL($11::text[])`,
            r.ext, r.companyId, r.placa, r.tipovhc, r.configuracion, r.kilometrajeActual, r.estadoOperacional, r.fueraDeOperacionDesde, r.originalClient, JSON.stringify(r.sourceMetadata), remaxArr,
          ), 'veh.single');
        } catch (e2: any) {
          if (!isUniqueViolation(e2)) throw e2;
          // (companyId, placa) collision with row that has DIFFERENT ext.
          // Update that row's data only (keep its current extId).
          await retry(() => prisma.$executeRawUnsafe(
            `UPDATE "Vehicle" SET tipovhc=$1, configuracion=$2, "kilometrajeActual"=$3, "estadoOperacional"=$4::"VehicleOperationalState", "fueraDeOperacionDesde"=$5, "originalClient"=$6, "sourceMetadata"=$7::jsonb, "updatedAt"=NOW()
              WHERE placa=$8 AND "companyId"=$9
                AND ("companyId" IS NULL OR "companyId" <> ALL($10::text[]))`,
            r.tipovhc, r.configuracion, r.kilometrajeActual, r.estadoOperacional, r.fueraDeOperacionDesde, r.originalClient, JSON.stringify(r.sourceMetadata), r.placa, r.companyId, remaxArr,
          ), 'veh.update');
        }
      }
    }
  ...
  // Map vehicleId-num → (id, companyId, placa)
  const allMV = await prisma.vehicle.findMany({ where: { externalSourceId: { startsWith: 'merquepro:vehicle:' } }, select: { id: true, externalSourceId: true, companyId: true, placa: true } });
  ...
  for (let i = 0; i < tireRows.length; i += TBATCH) {
    ...
    const sql = `
      ...
      VALUES ${values.join(', ')}
      ON CONFLICT ("externalSourceId") DO UPDATE SET
        "companyId" = CASE WHEN "Tire"."companyId" = ANY(${remaxP}) THEN "Tire"."companyId" ELSE EXCLUDED."companyId" END,
        "vehicleId" = EXCLUDED."vehicleId",
        placa = EXCLUDED.placa, marca = EXCLUDED.marca, diseno = EXCLUDED.diseno, dimension = EXCLUDED.dimension,
        eje = EXCLUDED.eje, posicion = EXCLUDED.posicion,
        "profundidadInicial" = EXCLUDED."profundidadInicial",
        "vidaActual" = EXCLUDED."vidaActual", "totalVidas" = EXCLUDED."totalVidas",
        "kilometrosRecorridos" = EXCLUDED."kilometrosRecorridos",
        "currentCpk" = EXCLUDED."currentCpk", "currentProfundidad" = EXCLUDED."currentProfundidad",
        "fechaInstalacion" = EXCLUDED."fechaInstalacion",
        "sourceMetadata" = EXCLUDED."sourceMetadata",
        "updatedAt" = NOW()
      WHERE "Tire"."companyId" IS NULL OR "Tire"."companyId" <> ALL(${remaxP})`;
  ...
  // Tire ext → id map
  const allTires = await prisma.tire.findMany({ where: { externalSourceId: { startsWith: 'merquepro:tire:' } }, select: { id: true, externalSourceId: true, companyId: true } });
  ...
  for (let i = 0; i < inspRows.length; i += IBATCH) {
    ...
    const sql = `
      ...
      ON CONFLICT ("externalSourceId") DO UPDATE SET
        ...
        "profundidadInt" = EXCLUDED."profundidadInt",
        "profundidadCen" = EXCLUDED."profundidadCen",
        "profundidadExt" = EXCLUDED."profundidadExt",
        "presionPsi" = EXCLUDED."presionPsi",
        "kmActualVehiculo" = EXCLUDED."kmActualVehiculo",
        "inspeccionadoPorNombre" = EXCLUDED."inspeccionadoPorNombre",
        "vidaAlMomento" = EXCLUDED."vidaAlMomento",
        "sourceMetadata" = EXCLUDED."sourceMetadata"`;
  ...
  // ── COSTOS (one per tire, only if commercialCost > 0 and no existing) ──
  console.log('\nBULK costos…');
  const costRows: any[] = [];
  ...
  // Skip tires that already have any cost
  const haveCost = new Set<string>();
  if (costRows.length > 0) {
    const tireIds = costRows.map((c) => c.tireId);
    const exists: any[] = await prisma.$queryRaw`SELECT DISTINCT "tireId" FROM tire_costos WHERE "tireId" = ANY(${tireIds}::text[])`;
    for (const x of exists) haveCost.add(x.tireId);
  }
  const newCosts = costRows.filter((c) => !haveCost.has(c.tireId));
  console.log(`  costos to insert: ${newCosts.length}`);
  const CBATCH = 1000;
  for (let i = 0; i < newCosts.length; i += CBATCH) {
    const slice = newCosts.slice(i, i + CBATCH);
    const values: string[] = [];
    const params: any[] = [];
    let p = 1;
    for (const r of slice) {
    ...
    await retry(() => prisma.$executeRawUnsafe(
      `INSERT INTO tire_costos (id, "tireId", valor, fecha, concepto) VALUES ${values.join(', ')}`,
      ...params,
    ), 'costos');
  }

  // ── VIDA EVENTS (only where missing) ─────────────────────────────────
  await retry(() => prisma.$executeRawUnsafe(`
    ...
    SELECT gen_random_uuid()::text, t.id, 'montaje'::"TireEventType",
           ...
           jsonb_build_object('source','merquepro_rebuild_fast'), NOW()
      FROM "Tire" t
  ...
  const currentExts = [...tireById.keys()].map((n) => `merquepro:tire:${n}`);
  const finUpd = await retry(() => prisma.$executeRawUnsafe(`
    UPDATE "Tire" SET "vidaActual" = 'fin'::"VidaValue"
     WHERE "externalSourceId" LIKE 'merquepro:tire:%'
       AND "externalSourceId" <> ALL($1::text[])
       AND ("companyId" IS NULL OR "companyId" <> ALL($2::text[]))
       AND "vidaActual" <> 'fin'`, currentExts, remaxArr), 'fin');
  console.log(`Tires marked fin: ${finUpd}`);
